...
// var hasInnerText = (_e, _innerTextHas, _level) => {
//   if (!_e) {
//     return false
//   }
//   if (_e.innerText.includes(_innerTextHas)) {
//     return true
//   }

//   if (Number.isInteger(_level) && _level > 0) {
//     if (_level > 10) {
//       _level = 10
//     }
//     let p = _e
...
//         return false
//       }
//       if (p.innerText.includes(_innerTextHas)) {
//         return true
//       }
//     }
//   }
...
var focusSelectorEx = function(done, selector, innerTextHas, level) {
  return this.evaluate_now(
    function(__selector, __innerText, __level) {
      const hasInnerText = (_e, _innerTextHas, _level) => {
        if (!_e) {
          return false
        }
        if (_e.innerText.includes(_innerTextHas)) {
      ...
      const list = document.querySelectorAll(__selector)
      for (const e of list) {
        if (hasInnerText(e, __innerText, __level)) {
          e.focus()
          break
        }
      }
...
var blurSelectorEx = function(done, selector, innerTextHas, level) {
  return this.evaluate_now(
    function(__selector, __innerText, __level) {
      const hasInnerText = (_e, _innerTextHas, _level) => {
        if (!_e) {
...
/**
 * TypeEx into an element.
 *
 * @param {String} selector
 * @param {String} text
 * @param {String} paramInnerTextHas
 * @param {number} paramLevel
 * @param {Function} done
 */
exports.typeEx = function(
  paramSelector,
  paramText,
  paramInnerTextHas,
  paramLevel,
  ...
  debug(
    '.typeEx() %s into %s innerText %s level %d',
    paramText,
    paramSelector,
    paramInnerTextHas,
    paramLevel
  ...
  var self = this
  focusSelectorEx.bind(this)(
    function(err) {
      if (err) {
        debug('Unable to .type() into non-existent selector %s', paramSelector)
        return paramDone(err)
      }

      var blurDone = blurSelectorEx.bind(
        this,
        paramDone,
        paramSelector,
        paramInnerTextHas,
      ...
      if ((paramText || '') == '') {
        ...
        self.child.call('type', paramText, blurDone)
      }
    },
    paramSelector,
    paramInnerTextHas,
...
exports.type = function() {
  ...
  focusSelector.bind(this)(function(err) {
    if (err) {
      ...
      return done({ msg: 'focusSelector.bind' + selector, err })
    }
...
exports.select_index = function(selector, option, index, done) {
  debug('.select_index() ' + selector)
  this.evaluate_now(
    function(selector, option, index) {
